...
class WindowsManifest(ViewerManifest):
    def final_exe(self):
        if self.default_channel():
            if self.default_grid():
                return "LifeStream.exe"
            else:
                return "LifeStreampreview.exe"
        else:
    ...
    def construct(self):
        ...
        self.path(self.find_existing_file('debug/LifeStream-bin.exe', 'release/LifeStream-bin.exe', 'relwithdebinfo/LifeStream-bin.exe'), dst=self.final_exe())
        ...
        self.path(src="%s/LifeStream-bin.exe.config" % self.args['configuration'], dst=self.final_exe() + ".config")

        # We need this one too, so that llkdu loads at runtime - DEV-41194
        #self.path(src="%s/LifeStream-bin.exe.config" % self.args['configuration'], dst="llkdu.dll.2.config")
        self.path("llkdu.dll.2.config")

        # We need this one too, so that win_crash_logger.exe loads at runtime - DEV-19004
        #self.path(src="%s/LifeStream-bin.exe.config" % self.args['configuration'], dst="win_crash_logger.exe.config")

        # same thing for auto-updater.
        #self.path(src="%s/LifeStream-bin.exe.config" % self.args['configuration'], dst="updater.exe.config")
...
#                  dst="win_crash_logger.exe")
    ...
    def package_finish(self):
        ...
        if self.default_channel():
            if self.default_grid():
                # release viewer
                installer_file = "LifeStream_%(version_dashes)s_Setup.exe"
                grid_vars_template = """
                OutFile "%(installer_file)s"
                !define INSTFLAGS "%(flags)s"
                !define INSTNAME   "LifeStream"
                !define SHORTCUT   "LifeStream"
                !define URLNAME   "LifeStream"
                Caption "LifeStream ${VERSION}"
                """
            else:
                # beta grid viewer
                installer_file = "LifeStream_%(version_dashes)s_(%(grid_caps)s)_Setup.exe"
                grid_vars_template = """
                OutFile "%(installer_file)s"
                !define INSTFLAGS "%(flags)s"
                !define INSTNAME   "LifeStream%(grid_caps)s"
                !define SHORTCUT   "LifeStream (%(grid_caps)s)"
                !define URLNAME   "LifeStream%(grid)s"
                !define UNINSTALL_SETTINGS 1
                Caption "LifeStream %(grid)s ${VERSION}"
                """
        else:
            # some other channel on some grid
            installer_file = "LifeStream_%(version_dashes)s_%(channel_oneword)s_Setup.exe"
            grid_vars_template = """
            OutFile "%(installer_file)s"
            !define INSTFLAGS "%(flags)s"
            !define INSTNAME   "LifeStream%(channel_oneword)s"
        ...
        tempfile = "LifeStream_setup_tmp.nsi"
...
class DarwinManifest(ViewerManifest):
    def construct(self):
        ...
        self.path(self.args['configuration'] + "/LifeStream.app", dst="")
    ...
    def package_finish(self):
        channel_standin = 'LifeStream'  # hah, our default channel is not usable on its own
        if not self.default_channel():
            channel_standin = self.channel()

        imagename="LifeStream_" + '_'.join(self.args['version'])

        # MBW -- If the mounted volume name changes, it breaks the .DS_Store's background image and icon positioning.
        #  If we really need differently named volumes, we'll need to create multiple DS_Store file images, or use some other trick.

        volname="LifeStream Installer"  # DO NOT CHANGE without understanding comment above
        ...
        if self.default_channel() and not self.default_grid():
            app_name = "LifeStream " + self.args['grid']
        else:
...
class LinuxManifest(ViewerManifest):
    def construct(self):
        ...
        self.path("res/LifeStream_icon.png","LifeStream_icon.png")
        if self.prefix("linux_tools", dst=""):
            self.path("alsoft.conf")
            #self.path("client-readme.txt","README-linux.txt")
            self.path("client-readme-voice.txt","README-linux-voice.txt")
            #self.path("client-readme-joystick.txt","README-linux-joystick.txt")
            self.path("wrapper.sh","LifeStream")
    ...
    def package_finish(self):
        if 'installer_name' in self.args:
            ...
            installer_name_components = ['LifeStream_', self.args.get('arch')]
...
class Linux_i686Manifest(LinuxManifest):
    def construct(self):
        ...
        if self.debug():
            self.path("LifeStream-bin","bin/do-not-directly-run-LifeStream-bin")
        else:
            self.path("LifeStream-stripped","bin/do-not-directly-run-LifeStream-bin")
